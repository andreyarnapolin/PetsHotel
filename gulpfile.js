...
// Modules
function modulesCSS() {
  return gulp.src(sources.css).pipe(gulp.dest("app/css/libs"));
}

function modulesJS() {
  return gulp.src(sources.js).pipe(gulp.dest("app/js/libs"));
}
...
function images() {
  return gulp
    .src(`${paths.images.src}.+(png|jpg|jpeg|gif)`)
    .pipe(
      cache(
        imagemin([
...
const build = gulp.series(
  ...
  modules,
  styles,
  gulp.parallel(distFiles, images, imagesSVG, fonts)
);
